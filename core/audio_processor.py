...
class AudioProcessor:
    def __init__(self, config: dict, progress_callback=None, 
        ...
        try:
            ...
            if self.progress_callback: self.progress_callback("Initialization complete.", 100)
        ...
        try:
            ...
            if is_plain_text:
                ...
                return ProcessedAudioResult(
                    ...
                    data=" ".join([s['text'].strip() for s in transcription_result['segments']]), 
                    is_plain_text_output=True
                )
            ...
            final_text = self._format_segment_dictionaries_to_strings(
                aligned_segments, 
                self.output_include_timestamps, 
                self.output_include_end_times, 
                diarization_was_performed
            )
    ...
    def _format_segment_dictionaries_to_strings(self, segment_dicts, include_ts, include_end_ts, include_speakers):
        lines = []
        for seg_dict in segment_dicts:
            parts = []
            if include_ts:
