...
from PySide6.QtWidgets import (QFileDialog, QMessageBox, QVBoxLayout, QColorDialog, QDialog, 
                               ...
                               QWidget, QComboBox, QRadioButton, QHBoxLayout)
...
class CorrectionViewLogic(QObject):
    def __init__(self, main_window):
        super().__init__()
        self.main_window = main_window; self.segment_manager = SegmentManager(); self.audio_player = AudioPlayer()
        self.selected_segment_id = None; self.current_highlighted_segment_id = None; self.editing_segment_id = None
        self.timestamp_editing_segment_id = None; self.multi_selection_ids = []
        self.normal_format = QTextCharFormat(); self.highlight_format = QTextCharFormat()
        self.selection_format = QTextCharFormat(); self.multi_selection_format = QTextCharFormat()
    ...
    def connect_audio_player_signals(self):
        self.audio_player.progress.connect(self.update_audio_progress);self.audio_player.finished.connect(self.on_audio_finished);self.audio_player.error.connect(lambda msg: QMessageBox.critical(self.main_window, "Audio Player Error", msg));self.audio_player.state_changed.connect(self.update_play_button_state)

    def connect_signals(self):
        textarea = self.main_window.correction_text_area
        if textarea: textarea.segment_clicked.connect(self.on_segment_clicked); textarea.edit_requested.connect(self.on_edit_requested); textarea.edit_cancelled.connect(lambda: self.exit_edit_mode(save=False))
    ...
    def on_delete_segment_clicked(self):
        ...
        if QMessageBox.question(self.main_window, "Confirm Delete", f"Are you sure you want to delete {len(target_ids)} segment(s)?") == QMessageBox.Yes:
            for seg_id in target_ids: self.segment_manager.remove_segment(seg_id)
            self._clear_all_selections(); self.render_segments_to_textarea(); self.update_edit_buttons_state()
    ...
    def on_add_split_button_clicked(self):
        if self.editing_segment_id is not None:
            ...
            block_number = self.segment_manager.get_segment_index(self.editing_segment_id)
            current_line_text = self.main_window.correction_text_area.document().findBlockByNumber(block_number).text()
            self.segment_manager.update_segment_from_full_line(self.editing_segment_id, current_line_text)
            ...
            defaults = {"speaker": original_segment.get("speaker_raw"), "has_timestamps": original_segment.get("has_timestamps"), "has_explicit_end_time": original_segment.get("has_explicit_end_time", False)}
            ...
            if result:
                if self.segment_manager.split_segment(self.editing_segment_id, split_pos, result):
                    self.exit_edit_mode(save=False); self.render_segments_to_textarea()
                else: self.render_segments_to_textarea()
        elif self.selected_segment_id is not None:
            result = self._open_add_split_dialog(is_split_mode=False)
            if result: self.segment_manager.add_segment(result, self.selected_segment_id, result['position']); self.render_segments_to_textarea()
    ...
    def _open_add_split_dialog(self, is_split_mode, defaults={}):
        ...
        default_speaker = defaults.get('speaker', constants.NO_SPEAKER_LABEL) if is_split_mode else constants.NO_SPEAKER_LABEL
    ...
    def render_segments_to_textarea(self):
        textarea = self.main_window.correction_text_area; current_selection_id=self.selected_segment_id;
        textarea.clear(); self.current_highlighted_segment_id = None
        ...
        for seg in self.segment_manager.segments:
            seg['doc_positions']=(cursor.position(),); seg['component_positions'] = {}
            if seg.get("has_timestamps"):
                ts_start_pos=cursor.position(); ts_str=f"[{self.segment_manager.seconds_to_time_str(seg['start_time'])}] "
                cursor.insertText(ts_str, self.normal_format); seg['component_positions']['timestamp'] = (ts_start_pos, cursor.position())
    ...
    def update_edit_buttons_state(self):
        ...
        self.main_window.delete_segment_btn.setEnabled(is_selected or is_multi_selected)
        self.main_window.merge_segments_btn.setEnabled(bool(len(self.multi_selection_ids) > 1 or (is_selected and self.segment_manager.get_segment_index(self.selected_segment_id) > 0)))
    ...
    def exit_timestamp_edit_mode(self, save=False):
        ...
        if save:
            new_start_time = self.timeline.start_bar_pos_secs; time_str = self.segment_manager.seconds_to_time_str(new_start_time)
            self.segment_manager.update_segment_timestamps(self.timestamp_editing_segment_id, time_str, None)
            self.render_segments_to_textarea(); QMessageBox.information(self.main_window, "Timestamp Saved", f"Segment start time updated to {time_str}")
        ...
        self.timestamp_editing_segment_id = None; self.timeline.exit_edit_mode(); self.update_edit_buttons_state()
    def on_seek_button_clicked(self, is_forward: bool):
        offset = 1.0 if is_forward else -1.0 if self.timestamp_editing_segment_id else 5.0 if is_forward else -5.0
    ...
    def on_segment_clicked(self, block_number, modifiers):
        ...
        if modifiers == Qt.KeyboardModifier.ShiftModifier and is_click_on_valid_segment:
            ...
            if self.selected_segment_id: self._clear_selection()
            if segment_id in self.multi_selection_ids: self.multi_selection_ids.remove(segment_id); self._apply_format(segment_id, self.normal_format)
            else: self.multi_selection_ids.append(segment_id); self._apply_format(segment_id, self.multi_selection_format)
        else:
    ...
    def _clear_all_selections(self):
        ...
        for seg_id in self.multi_selection_ids: self._apply_format(seg_id, self.normal_format)
        self.multi_selection_ids.clear()
    @Slot()
    def on_merge_button_clicked(self):
        self.exit_all_edit_modes(); num_multi_selected = len(self.multi_selection_ids)
        if num_multi_selected > 1:
            new_target_id = self.segment_manager.merge_multiple_segments(self.multi_selection_ids)
            self._clear_all_selections(); self.render_segments_to_textarea()
            if new_target_id: self.select_segment(new_target_id)
        elif self.selected_segment_id and num_multi_selected == 0:
            current_id = self.selected_segment_id; current_index = self.segment_manager.get_segment_index(current_id)
            if current_index > 0:
                previous_id = self.segment_manager.segments[current_index - 1]['id']
                if self.segment_manager.merge_segment_upwards(current_id):
                    self._clear_all_selections(); self.render_segments_to_textarea(); self.select_segment(previous_id)
        self.update_edit_buttons_state()
    @Slot()
    ...
    def _apply_format(self, segment_id, text_format, clear_first=False):
        ...
        if segment and 'doc_positions' in segment:
            ...
            cursor.setPosition(segment['doc_positions'][0]); cursor.setPosition(segment['doc_positions'][1], QTextCursor.MoveMode.KeepAnchor)
    ...
    def _clear_selection(self):
        if self.selected_segment_id: is_highlighted = self.selected_segment_id == self.current_highlighted_segment_id; self._apply_format(self.selected_segment_id, self.highlight_format if is_highlighted else self.normal_format)
        self.selected_segment_id = None
    def _clear_highlight(self):
        if self.current_highlighted_segment_id: is_selected = self.current_highlighted_segment_id == self.selected_segment_id; self._apply_format(self.current_highlighted_segment_id, self.selection_format if is_selected else self.normal_format)
    @Slot()
    def on_edit_speaker_clicked(self): self._safe_action(self._open_change_speaker_dialog)
    def _open_change_speaker_dialog(self):
        if not self.selected_segment_id: return
        segment = self.segment_manager.get_segment_by_id(self.selected_segment_id)
        if not segment: return
        dialog=QDialog(self.main_window); dialog.setWindowTitle("Change Segment Speaker"); layout=QVBoxLayout(dialog); layout.addWidget(QLabel("Assign an existing speaker to this segment:"))
        combo = QComboBox(); combo.addItem("(No Speaker)", constants.NO_SPEAKER_LABEL)
        for speaker_id in sorted(list(self.segment_manager.unique_speaker_labels)): combo.addItem(f"{self.segment_manager.speaker_map.get(speaker_id, speaker_id)} ({speaker_id})", speaker_id)
        layout.addWidget(combo); current_speaker_index = combo.findData(segment.get("speaker_raw", constants.NO_SPEAKER_LABEL))
        ...
        if dialog.exec() == QDialog.Accepted:
            self.segment_manager.update_segment_speaker(self.selected_segment_id, combo.currentData()); self.render_segments_to_textarea()
    ...
    def exit_edit_mode(self, save=False):
        ...
        if save:
            ...
            if block_number != -1: self.segment_manager.update_segment_from_full_line(segment_id_to_exit, self.main_window.correction_text_area.document().findBlockByNumber(block_number).text())
        self.editing_segment_id=None; self.main_window.correction_text_area.exit_edit_mode()
        self.render_segments_to_textarea() # Must be called after exit_edit_mode
    ...
    def _save_changes_action(self):
        ...
        if path:
            try:
                with open(path, 'w', encoding='utf-8') as f: f.write('\n'.join(self.segment_manager.format_segments_for_saving(True, True)))
    ...
    def _update_text_highlight(self, current_time):
        ...
        if self.audio_player.get_duration() > 0:
            for i, seg in enumerate(self.segment_manager.segments):
                ...
                start_time=seg.get('start_time', -1); end_time=seg.get('end_time')
                if end_time is None:
                    if (i + 1) < len(self.segment_manager.segments) and self.segment_manager.segments[i+1].get('has_timestamps'): end_time=self.segment_manager.segments[i+1].get('start_time')
