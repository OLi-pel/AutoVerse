...
class CorrectionViewLogic(QObject):
    ...
    def _open_change_speaker_dialog(self, segment, target_ids):
        ...
        delete_speaker_button = QPushButton()
        trash_icon = self.main_window.delete_segment_btn.icon()
        delete_speaker_button.setIcon(trash_icon)
        ...
        def on_accept():
            new_speaker = combo.currentData()
            for seg_id in target_ids:
                self.segment_manager.update_segment_speaker(seg_id, new_speaker)
            dialog.done(QDialog.Accepted)
            
        def on_delete_speaker():
            for seg_id in target_ids:
                self.segment_manager.update_segment_speaker(seg_id, constants.NO_SPEAKER_LABEL)
            dialog.done(QDialog.Accepted)

        button_box.accepted.connect(on_accept)
        button_box.rejected.connect(dialog.reject)
        delete_speaker_button.clicked.connect(on_delete_speaker)
    ...
    def enter_edit_mode(self, segment_id, click_pos_in_block: int = 0):
        ...
        if not segment_obj or block_number == -1: return

        # --- THIS IS THE NEW, CORRECTED LOGIC ---
        # 1. Check the MODEL data for the placeholder.
        if segment_obj.get('text') == constants.EMPTY_SEGMENT_PLACEHOLDER:
            # 2. If it matches, find the text part in the VIEW and clear it.
            ...
            # 3. Force the click position to be at the start of the now-empty text area.
            click_pos_in_block = segment_obj.get('component_positions', {}).get('text', (0,0))[0]
        # --- END OF FIX ---
    ...
    def _update_text_highlight(self, current_time):
        ...
        if self.audio_player.get_duration() > 0:
            for i, seg in enumerate(self.segment_manager.segments):
                ...
                # --- THIS IS THE RESTORED, CORRECTED LOGIC ---
