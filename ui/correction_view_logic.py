...
class CorrectionViewLogic(QObject):
    def __init__(self, main_window, main_app=None):
        ...
        self.tip_widgets = {
            ...
            self.main_window.font_size_combo: "Police_size",
    ...
    def connect_audio_player_signals(self):
        ...
        self.audio_player.crashed.connect(self._recover_audio_player)
    ...
    def toggle_play_pause(self):
        ...
        is_going_to_play = not self.audio_player.is_playing
        if is_going_to_play:
            current_device_name = AudioPlayer.get_current_device_name()
    ...
    def on_delete_segment_clicked(self):
        ...
        if self.timestamp_editing_segment_id:
            if QMessageBox.question(self.main_window, "Confirm Delete Timestamp", "Remove timestamp?") == QMessageBox.Yes:
                self.segment_manager.remove_segment_timestamp(self.timestamp_editing_segment_id); self.exit_timestamp_edit_mode(save=False); confirmed_action = True
        elif self.editing_segment_id:
            if QMessageBox.question(self.main_window, "Confirm Clear Text", "Clear text content?") == QMessageBox.Yes:
                self.segment_manager.clear_segment_text(self.editing_segment_id); self.exit_edit_mode(save=False); confirmed_action = True
        else:
            ...
            if target_ids and QMessageBox.question(self.main_window, "Confirm Delete", f"Delete {len(target_ids)} segment(s)?") == QMessageBox.Yes:
    ...
    def on_add_split_button_clicked(self):
        ...
        if self.editing_segment_id is not None:
            ...
            if result and self.segment_manager.split_segment(self.editing_segment_id, split_pos, result):
    ...
    def _open_add_split_dialog(self, is_split_mode, defaults={}):
        ...
        speaker_combo.setCurrentIndex(speaker_combo.findData(defaults.get('speaker_raw', constants.NO_SPEAKER_LABEL))); layout.addWidget(speaker_combo, 0, 1)
    ...
    def render_segments_to_textarea(self):
        ...
        self.main_window.correction_text_area.clear(); self.current_highlighted_segment_id = None
        if not self.segment_manager.segments: self.main_window.correction_text_area.setPlainText("No segments loaded."); return
        ...
        for seg in self.segment_manager.segments:
            ...
            if seg.get("has_timestamps"):
                ...
                if seg.get('has_explicit_end_time') and seg.get('end_time') is not None: ts_str = f"[{self.segment_manager.seconds_to_time_str(seg['start_time'])} - {self.segment_manager.seconds_to_time_str(seg['end_time'])}] "
    ...
    def update_edit_buttons_state(self):
        ...
        self.main_window.delete_segment_btn.setEnabled(is_text_editing or is_ts_editing or is_selected or is_multi_selected)
    ...
    def exit_timestamp_edit_mode(self, save=False):
        ...
        if save:
            ...
            QMessageBox.information(self.main_window, "Timestamp Saved", f"Segment start time updated to {time_str}"); self.render_segments_to_textarea()
        ...
        if self.segment_manager.get_segment_by_id(id_to_reselect): self.select_segment(id_to_reselect)
    ...
    def on_segment_clicked(self, block_number, modifiers):
        ...
        if self.editing_segment_id or self.timestamp_editing_segment_id:
            ...
            if self.editing_segment_id == clicked_id or self.timestamp_editing_segment_id == clicked_id: return
        ...
        if is_shift_pressed:
            if self.selected_segment_id and self.selected_segment_id != segment_id:
                 ...
                 self._clear_all_selections(update_buttons=False); self.multi_selection_ids = new_multi_ids
                 ...
                 self.multi_selection_ids.append(segment_id); self._apply_format(segment_id, self.multi_selection_format)
            else:
                self.multi_selection_ids.remove(segment_id); self._apply_format(segment_id, self.normal_format)
        else: self._clear_all_selections(); self.select_segment_by_block(block_number)
        self.update_edit_buttons_state()
    ...
    def on_merge_button_clicked(self):
        ...
        self.exit_all_edit_modes(); action_made = False; num_multi_selected = len(self.multi_selection_ids); new_target_id = None
        if num_multi_selected > 1:
            ...
            if current_index > 0:
                ...
                if self.segment_manager.merge_segment_upwards(current_id): new_target_id = previous_id; action_made = True
        if action_made:
            def action():
    ...
    def _open_change_speaker_dialog(self, segment, target_ids):
        ...
        delete_speaker_button.setToolTip("Remove speaker assignment"); delete_speaker_button.setFixedSize(QSize(32, 32)); delete_speaker_button.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        def on_accept(speaker_to_set):
            def action():
                for seg_id in target_ids: self.segment_manager.update_segment_speaker(seg_id, speaker_to_set)
    ...
    def on_edit_requested(self, block_number, position_in_block):
        ...
        segment = self.segment_manager.segments[block_number]; block = self.main_window.correction_text_area.document().findBlockByNumber(block_number); abs_pos = block.position() + position_in_block
        if 'component_positions' in segment:
            positions = segment['component_positions']
            if 'speaker' in positions and positions['speaker'][0] <= abs_pos < positions['speaker'][1]: self.select_segment(segment['id']); self._safe_action(self._open_change_speaker_dialog, segment, [segment['id']]); return
    ...
    def enter_edit_mode(self, segment_id, click_pos_in_block: int = 0):
        ...
        self.original_text_before_edit = segment_obj.get('text'); self.editing_segment_id = segment_id
        ...
        if segment_obj.get('text') == constants.EMPTY_SEGMENT_PLACEHOLDER:
            ...
            if block.isValid() and 'text' in segment_obj.get('component_positions', {}):
                ...
                cursor = QTextCursor(block); cursor.setPosition(text_start); cursor.setPosition(text_end, QTextCursor.MoveMode.KeepAnchor)
                cursor.removeSelectedText(); click_pos_in_block = cursor.position() - block.position()
        self.main_window.correction_text_area.enter_edit_mode(block_number, click_pos_in_block)
        self.select_segment(segment_id); self.update_edit_buttons_state()
    ...
    def exit_edit_mode(self, save=False):
        ...
        if save:
            ...
            segment_after = self.segment_manager.get_segment_by_id(segment_id_to_exit)
            if segment_after and self.original_text_before_edit != segment_after.get('text'): self._execute_command(before_segs, before_map, lambda: self.render_segments_to_textarea())
        self.editing_segment_id = None; self.original_text_before_edit = None; self.main_window.correction_text_area.exit_edit_mode()
        if not save: self.render_segments_to_textarea()
        if self.segment_manager.get_segment_by_id(segment_id_to_exit): self.select_segment(segment_id_to_exit)
        else: self._clear_all_selections()
        self.update_edit_buttons_state()
    ...
    def _update_text_highlight(self, current_time):
        ...
        if self.audio_player.get_duration() > 0:
            for i, seg in enumerate(self.segment_manager.segments):
                ...
                if end_time is None:
                    is_last = (i + 1) >= len(self.segment_manager.segments)
                    if not is_last and self.segment_manager.segments[i + 1].get('has_timestamps'): end_time = self.segment_manager.segments[i+1].get('start_time')
                    else: end_time = self.audio_player.get_duration()
                if end_time is not None and start_time <= current_time < end_time:
                    active_id = seg['id']; break
        if self.current_highlighted_segment_id != active_id:
            old_id = self.current_highlighted_segment_id; self.current_highlighted_segment_id = active_id
            if old_id:
                revert_format = self.selection_format if old_id == self.selected_segment_id else self.normal_format
    ...
    def _recover_audio_player(self):
        ...
        if not self.current_audio_path or not self.current_txt_path:
            logger.error("Cannot recover audio player: file paths not available."); return
        self.set_controls_enabled(False)
        current_time = self.audio_player._current_time
